...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, TwoWire *twi,
                                   uint8_t rst_pin, uint32_t clkDuring,
                                   uint32_t clkAfter)
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, uint8_t mosi_pin,
                                   uint8_t sclk_pin, uint8_t dc_pin,
...
// LC: Additional constructor with column offset, to accommodate 64x48 modules
...
/*!
    ...
    @param  h
            Display height in pixels
    @param  o
            Column offset in pixels (for displays where actual columns don't start from 0)
    @param  spi
            Pointer to an existing SPIClass instance (e.g. &SPI, the
            microcontroller's primary SPI bus).
    @param  dc_pin
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, uint8_t o, SPIClass *spi,
                                   uint8_t dc_pin, uint8_t rst_pin, uint8_t cs_pin,
                                   uint32_t bitrate)
    : Adafruit_GFX(w, h), offset(o), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
      mosiPin(-1), clkPin(-1), dcPin(dc_pin), csPin(cs_pin), rstPin(rst_pin) {
#ifdef SPI_HAS_TRANSACTION
  spiSettings = SPISettings(bitrate, MSBFIRST, SPI_MODE0);
#endif
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  // LC: Not sure yet why this doesn't work
  //    IIRC this was a bug where pin number was a signed char and rolled over
  if (reset && (rstPin >= 0)) {
  ...
  ssd1306_commandList(init1, sizeof(init1)); // 3F for 64 height
  ssd1306_command1(47); // HEIGHT - 1); // 

  static const uint8_t PROGMEM init2[] = {SSD1306_SETDISPLAYOFFSET, // 0xD3 - Vertical (row) offset
                                          0x0,                      // no offset
  ...
  static const uint8_t PROGMEM init3[] = {SSD1306_MEMORYMODE, // 0x20
                                          0x00, // 0x0 act like ks0108 LC: 00 = Horizontal addressing mode 02 = page address mode
                                          SSD1306_SEGREMAP | 0x1, // LC: A0 vs A1 - flips horizontal
                                          SSD1306_COMSCANDEC}; // LC: Need this? will flip vertical
  ssd1306_commandList(init3, sizeof(init3));


  //ssd1306_command1(0x00); // set column start address low nibble to 0x20 ** only for page addressing mode!
  //ssd1306_command1(0x12); // set column start address high nibble to 0x20 ** only for page addressing mode!

  // Set column address for 64 wide - Doesn't seem to do anything
  ssd1306_command1(SSD1306_COLUMNADDR); // 0x21
  ssd1306_command1(64);
  ssd1306_command1(127);
/*
  ssd1306_command1(SSD1306_PAGEADDR); // 0x22
  ssd1306_command1(0);
  ssd1306_command1(47);
*/
...
*/
  ...
  ssd1306_command1(comPins); // comPins);
  ssd1306_command1(SSD1306_SETCONTRAST); // 0x81
  ssd1306_command1(contrast); // 0xAF

  ssd1306_command1(SSD1306_SETPRECHARGE); // 0xd9
  ssd1306_command1((vccstate == SSD1306_EXTERNALVCC) ? 0x22 : 0xF1);
  static const uint8_t PROGMEM init5[] = {
      SSD1306_SETVCOMDETECT, // 0xDB
      0x20, // 0x40, // LC: Invalid?? Should be 0x20? Voltage level
      SSD1306_DISPLAYALLON_RESUME, // 0xA4
      SSD1306_NORMALDISPLAY,       // 0xA6
      SSD1306_DEACTIVATE_SCROLL, // 0x2E
      SSD1306_CHARGEPUMP,        // 0x8D // LC: Moved from Init2
      ((vccstate == SSD1306_EXTERNALVCC) ? 0x10 : 0x14),
      SSD1306_DISPLAYON, // Main screen turn on 0XA5
      SSD1306_NOP}; // LC: Added, seems to lose second last byte otherwise
...
/*!
    ...
    LC: Modified column & row start & end to work with 64x48 centre wired display
    LC: Add column offset in constructor and in here
*/
void Adafruit_SSD1306::display(void) {
  ...
  static const uint8_t PROGMEM dlist1[] = {
      ...
      HEIGHT - 1, //0xFF,                   // Page end LC: Fixed this to be height - 1
      SSD1306_COLUMNADDR, offset};          // Column start address LC: added offset
  ssd1306_commandList(dlist1, sizeof(dlist1));
  ssd1306_command1(WIDTH + offset - 1);     // Column end address
